...
def parser():
    ...
    doctor = commands.add_parser(
        "doctor", help="Discover local sources and check prerequisites (read-only)"
    )
    doctor.add_argument("--source")
    doctor.add_argument(
        "--codex", default="codex", help="Codex executable, pinned to " + CODEX_VERSION
    )
    listing = commands.add_parser("list", help="List source session IDs (read-only)")
    listing.add_argument("--source", required=True)
    listing.add_argument(
        "--key-env",
        help="Name of environment variable containing an authorized SQLCipher raw hex key",
    )
    for name in ("plan", "convert"):
        cmd = commands.add_parser(
            name,
            help="Inspect conversion without writing"
            if name == "plan"
            else "Build a portable migration bundle",
        )
        cmd.add_argument(
            "--source",
            required=True,
            help="TRAE user-data root, database.db, or structured JSON export",
        )
        cmd.add_argument(
            "--session",
            action="append",
            dest="session_ids",
            help="Source session ID; repeat to select multiple",
        )
        cmd.add_argument(
            "--cwd", help="Absolute local project path override; required when database has no cwd"
        )
        cmd.add_argument(
            "--history-mode",
            choices=("increments", "snapshots"),
            help="Override source record semantics; never guess cumulative snapshots",
        )
        cmd.add_argument("--key-env", help="Environment variable name, never the key itself")
        if name == "convert":
            cmd.add_argument(
                "--output", required=True, help="New directory, never an existing Codex home"
            )
            cmd.add_argument(
                "--accept-sensitive",
                action="store_true",
                help="Explicitly preserve detected possible secrets in the private local bundle",
            )
    verify = commands.add_parser(
        "verify", help="Test with real Codex and an offline loopback model, in isolation"
    )
    verify.add_argument("bundle")
    verify.add_argument("--codex", default="codex")
    install = commands.add_parser(
        "install", help="Dry-run by default; verify then install native rollouts"
    )
    ...
    install.add_argument(
        "--apply", action="store_true", help="Write after verification; close Codex first"
    )
    install.add_argument(
        "--allow-loss",
        action="store_true",
        help="Acknowledge reviewed lossy mappings in report.json",
    )
    undo = commands.add_parser(
        "rollback", help="Remove only unchanged imported rollouts; dry-run by default"
    )
    undo.add_argument("--codex-home", required=True)
    undo.add_argument("--bundle-id", required=True)
    undo.add_argument(
        "--apply", action="store_true", help="Close Codex before removing imported rollouts"
    )
    return root
...
def run(args):
    ...
    if args.command in ("plan", "convert"):
        ...
        return build_bundle(
            args.source, args.output, accept_sensitive=args.accept_sensitive, **options
        )
...
def main(argv=None):
    ...
    try:
        ...
        print(
            json.dumps(
                {
                    "error": f"Operation failed ({type(error).__name__}). Check paths, permissions, and the documented source schema."
                }
            ),
            file=sys.stderr,
        )
