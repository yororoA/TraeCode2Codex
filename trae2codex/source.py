...
from .common import MigrationError, fingerprint, load_json, timestamp
...
def discover():
    ...
    for root in roots:
        for name in names:
            ...
            if db.is_file() and str(db) not in found:
                found.append(str(db))
    return found
...
def probe(path):
    ...
    if path.suffix.lower() == ".json":
        kind = "json"
    return {
        "path": str(path), "kind": kind, "bytes": path.stat().st_size,
...
def connect_readonly(path, key_env=None):
    ...
    if info["kind"] == "encrypted-or-unknown":
        ...
        if not re.fullmatch(r"[0-9a-fA-F]{64}", key):
            raise MigrationError("--key-env must contain a raw 32-byte SQLCipher key (64 hex characters).")
        try:
            import sqlcipher3 as module
        except ImportError:
            raise MigrationError("SQLCipher binding unavailable. Install sqlcipher3 for your platform.") from None
    connection = None
    try:
...
def make_session(raw, cwd_override=None):
    ...
    if not isinstance(cwd, str) or not Path(cwd).is_absolute():
        raise MigrationError("Session cwd is absent or not local/absolute; supply --cwd /project/path.")
    ...
    for index, record in enumerate(records):
        ...
        if mode == "snapshots":
            ...
            added = messages[len(previous):]
            previous = messages
        else:
            if len(previous) > 1 and messages[:len(previous)] == previous:
                raise MigrationError("Cumulative history detected; specify history_mode=snapshots in JSON.")
            added = messages
            previous = messages
        for message_index, message in enumerate(added):
            entries.append({
                "message": message, "timestamp": time,
                "source_ref": "%s:%s" % (record_id or index, message_index),
            })
    if not entries:
        raise MigrationError("Selected session contains no messages.")
    return Session(
        source_id, str(raw.get("session_title", raw.get("title", source_id))),
        cwd, created, entries, raw, warnings,
    )


def load_source(path, session_ids=None, cwd=None, key_env=None):
    info = probe(path)
    ...
    if info["kind"] == "json":
        ...
        if not isinstance(document, dict) or document.get("format") != "trae2codex.source.v1":
            raise MigrationError("Unsupported JSON format. Expected trae2codex.source.v1; see docs/source-format.md.")
        ...
        candidates = [s for s in raw_sessions if not selected or s.get("session_id", s.get("id")) in selected]
    else:
        with connect_readonly(info["path"], key_env) as connection:
            tables = {row[0] for row in connection.execute("SELECT name FROM sqlite_master WHERE type='table'")}
            if not {"chat_session", "history_v2"}.issubset(tables):
                raise MigrationError("Unsupported database schema; chat_session and history_v2 are required.")
            ...
            metadata = rows(connection, "SELECT * FROM chat_session ORDER BY created_at, session_id")
            candidates = []
...
def list_source(path, key_env=None):
    ...
    if info["kind"] == "json":
        ...
        if not isinstance(doc.get("sessions"), list) or any(not isinstance(s, dict) for s in doc["sessions"]):
            raise MigrationError("Source sessions must be an array of objects.")
        return [{
            "session_id": s.get("session_id", s.get("id")),
            "title": s.get("session_title", s.get("title")), "created_at": s.get("created_at"),
    ...
    with connect_readonly(info["path"], key_env) as connection:
        ...
        return rows(connection, "SELECT session_id, " + title + " AS title, created_at "
